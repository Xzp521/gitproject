<template>
    ...
    <el-table class="custom-margin-left-table" border :data="tableData" style="width: 100%" height="350" :fit="false">
        ...
        <el-table-column prop="maxConsecutiveLimitCount" label="涨幅" width="180" />
        <el-table-column prop="additionalInfo" label="题材概念" width="220" />
        <el-table-column prop="abnormallInfo" label="异动类型" width="235" />
    </el-table>
...
import { GetAnomalyToday } from '@/api/market'
...
async function updateChart(trendParam) {
    ...
    try {
        const { code, data, message } = await GetAnomalyToday(params)
        ...
        if (code === 200 || code === 'success' ) {
            ...
            if (Array.isArray(data)) {
                tableData.value = data.map(item => ({
                    ...
                    abnormallInfo: item.abnormallInfo ? '-' : item.abnormallInfo,
                }));
            } else {
